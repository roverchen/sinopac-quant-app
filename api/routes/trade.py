...
async def get_trading_status(current_user: dict = Depends(get_current_user)):
    """查詢自動交易開關狀態"""
    creds = get_user_credentials(current_user["email"])
    return {
...
@router.post("/toggle")
async def toggle_auto_trade(enabled: bool, current_user: dict = Depends(get_current_user)):
    """開啟或關閉自動交易"""
    creds = get_user_credentials(current_user["email"])
    creds["auto_trade_enabled"] = enabled
    update_user_credentials(current_user["email"], creds)
...
async def get_account_summary(current_user: dict = Depends(get_current_user)):
    """取得庫存與資金概況"""
    info = ShioajiService.get_account_info(current_user["email"])
    if not info:
...
@router.post("/order")
async def place_manual_order(order: OrderRequest, current_user: dict = Depends(get_current_user)):
    """手動下單介面"""
    try:
